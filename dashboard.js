...
async function startMoodDetection() {
  ...
  console.log('📹 Camera initialized, starting 60-second detection...');
  
  // Detect emotions for 60 seconds
  const detectionStartTime = Date.now();
  const detectionDuration = 60000; // 60 seconds
  const emotions = [];
...
async function completeMoodDetection(emotions, video) {
  ...
  }, 60000); // Show result after additional 1 minute (2 minutes total)
}
